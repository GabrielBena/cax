    ...
    def __init__(
        ...
        *,
        ...
        kernel_init = [default_kernel_init for _ in hidden_layer_sizes] + [
            initializers.zeros_init() if init_do_nothing else default_kernel_init
        ]
        ...
        if input is not None:
            ...
            if len(input.shape) == 1:
                x_reshaped = jnp.reshape(
                    input, (*([1] * len(spatial_dims)), input.shape[-1])
                )
                x_broadcasted = jnp.broadcast_to(
                    x_reshaped, (*spatial_dims, input.shape[-1])
                )
            else:
                x_broadcasted = input
