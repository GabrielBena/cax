...
def state_to_target(state: State, n_channels_target: int) -> State:
    """Extract the 'alive' component from the state.
    ...
    """
    return state[..., -n_channels_target:]
...
def state_from_rgba_to_rgb(state: State) -> State:
    ...
    return (
        1.0 - alpha + rgb
    )  # (1.0 - alpha) * 1.0 + alpha * rgb, assume rgb has been multiplied by alpha
